...
function requestContextNow() {
  return (0, import_shared4.getRequestContext)()?.currentTime ?? Date.now();
}
...
var MiniflareCore = class extends import_shared12.TypedEventTarget {
  ...
  async #init(reloadAll = false) {
    ...
    const { compatibilityDate, compatibilityFlags, usageModel, globalAsyncIO } = options.CorePlugin;
    let ctxUpdate = this.#previousRootPath && this.#previousRootPath !== rootPath || this.#previousUsageModel !== usageModel || this.#previousGlobalAsyncIO !== globalAsyncIO || reloadAll;
